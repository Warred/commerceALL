...
public class ProduitResourceIT {
    ...
    public static Produit createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, TypeDeProduit.class).isEmpty()) {
            typeDeProduit = TypeDeProduitResourceIT.createEntity(em);
            em.persist(typeDeProduit);
            em.flush();
        } else {
            typeDeProduit = TestUtil.findAll(em, TypeDeProduit.class).get(0);
        }
        produit.setType(typeDeProduit);
        return produit;
    }
    /**
    ...
    public static Produit createUpdatedEntity(EntityManager em) {
        ...
        TypeDeProduit typeDeProduit;
        if (TestUtil.findAll(em, TypeDeProduit.class).isEmpty()) {
            typeDeProduit = TypeDeProduitResourceIT.createUpdatedEntity(em);
            em.persist(typeDeProduit);
            em.flush();
        } else {
            typeDeProduit = TestUtil.findAll(em, TypeDeProduit.class).get(0);
